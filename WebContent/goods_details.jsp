...
<html>
  <head>
	...
	function check(obj){
		//使用ajax实现异步颜色请求
		var type = obj.value;
		var price = parseInt(obj.nextElementSibling.getElementsByTagName("span")[1].innerText);
		var goodName = document.getElementsByName("good_name")[1].defaultValue;
		...
		$.ajax({
			...
			"data":{
				...
				"price":price,
				"goodName":goodName,
				"operate":"typeforcolor"
			},
			"success":function(data){
				var html = $("#color_temp").html();
				$(".colors").html("");
				for(var i = 0 ; i < data.length;i++){
					var repHtml = 
						html
						.replace(/{{index}}/g,i)
						.replace(/{{goodColor}}/g,data[i].goodColor);
					$(".colors").append(repHtml);
				}
			}
		})
...
<form action="GoodServlet?operate=buy" method="post" name = "form"> 
 <!--  <input type="hidden" id="operate" name="operate" value="buy"/> -->
  <input type="hidden" name="good_name" value="${good.goodName}"/>
	<div class="xiangqing">
		<div class="neirong w">
			<div class="xiaomi6 fl">${good.goodName}</div>
			<nav class="fr">
				<li><a href="">概述</a></li>
				<li>|</li>
				<li><a href="">变焦双摄</a></li>
	...
	<div class="jieshao mt20 w">
		<div class="left fl"><img class="detailimg" id="photo" src="${good.goodImg}"></div>
		<div class="right fr">
			  <div class="h3 ml20 mt20">${good.goodName}</div>
			  <div class="jianjie mr40 ml20 mt10">${good.goodDesc}</div>
			  <div class="jiage ml20 mt10">${good.goodPrice}元</div>
	    ...
	    <c:forEach items="${goodtList}" var="goods">
		        <div class="xzbb ml20 mt10 banbenb">
					<div class="banben fl">
						<a>
						    <input type="radio" id="type<%=count %>" name ="type" value=${goods.goodType} onclick="check(this)"/>
  ...
  <script type="text/html" id="color_temp">

	<div class="xzbb ml20 mt10 colorc" id="{{goodColor}}" >
				<div class="banben">
					<a>
					    <input type="radio" id="color{{index}}" name="color" value="{{goodColor}}" onclick="changeclo()" />
					    <label for="color{{index}}" style="cursor:pointer">
							<span>{{goodColor}}</span>
						</label>
					</a>
				</div>
